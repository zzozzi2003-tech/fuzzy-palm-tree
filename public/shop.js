...
async function loadStoreContent(){
  try{
    const data=await fetch('/api/store-content').then(r=>r.json());
    ['main','fast','smart'].forEach(key=>{
      const card=document.querySelector(`[data-showcase="${key}"]`);if(!card)return;
      const item=data?.[key]||{};
      const badge=card.querySelector('[data-showcase-badge]'),title=card.querySelector('[data-showcase-title]'),desc=card.querySelector('[data-showcase-description]'),img=card.querySelector('[data-showcase-image]');
      if(badge)badge.textContent=item.badge||'';if(title)title.textContent=item.title||'';if(desc){desc.textContent=item.description||'';desc.classList.toggle('hidden',!item.description)}
      if(img){img.style.backgroundImage=item.image?`linear-gradient(180deg,rgba(3,10,20,.05),rgba(3,10,20,.66)),url("${String(item.image).replaceAll('"','%22')}")`:'';img.classList.toggle('has-image',Boolean(item.image))}
    });
  }catch{}
}
