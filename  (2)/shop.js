...
function initPointerMotion(){
  ...
  const animate=()=>{
    ...
    if(Math.abs(tx-cx)>.001||Math.abs(ty-cy)>.001)raf=requestAnimationFrame(animate);else raf=0;
  };
...
function bindProductMotion(){
  ...
  cards.forEach((card,index)=>{
    const band=((index%4)+1)/4;
    card.dataset.motionStrength=(0.72+band*0.4).toFixed(2);
    ...
    card.addEventListener('pointerleave',()=>{
      ...
      card.style.setProperty('--card-x','50%');card.style.setProperty('--card-y','0%');
    });
  });
}

function bindShowcaseDrift(){
  if(window.matchMedia('(prefers-reduced-motion: reduce)').matches||!window.matchMedia('(pointer: fine)').matches)return;
  const section=document.querySelector('.store-main');
  ...
  function run(){
    ...
    grid.style.setProperty('--grid-shift-x', `${currentX.toFixed(2)}px`);
    grid.style.setProperty('--grid-shift-y', `${currentY.toFixed(2)}px`);
    const cards=grid.querySelectorAll('.product-card');
    cards.forEach(card=>{
      const strength=Number(card.dataset.motionStrength||0);
      const offsetX=(currentX*0.25*strength).toFixed(2)+'px';
      const offsetY=(currentY*0.18*strength).toFixed(2)+'px';
      card.style.setProperty('--move-x', offsetX);
      card.style.setProperty('--move-y', offsetY);
    });
  ...
  section.addEventListener('pointerleave',()=>{targetX=0;targetY=0;queue()});
}
