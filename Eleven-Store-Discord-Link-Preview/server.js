...
function absolutePreviewUrl(req,value,fallback="/discord-preview.png"){
...
app.get("/",(req,res)=>{
  ...
  sendPageWithPreview(req,res,"index.html",{
    ...
    image:absolutePreviewUrl(req,"/discord-preview.png"),
    url:origin+"/"
  });
});
...
app.get("/product/:slug",(req,res)=>{
  ...
  const product=products().find(p=>String(p?.slug||"")===slug&&p?.active!==false);
  if(!product)return res.status(404).sendFile(path.join(PUBLIC,"product.html"));
  const price=Number(product.price||0);
  const priceText=Number.isFinite(price)&&price>0?` • ${price} SAR`:"";
  const title=`${product.name||"Product"}${priceText} | Eleven Store`;
  const description=String(product.description||"Available now on Eleven Store.").replace(/\s+/g," ").trim().slice(0,220);
  const candidateImage=product.image||(Array.isArray(product.previewImages)&&product.previewImages[0])||"/discord-preview.png";
